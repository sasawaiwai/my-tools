...
function initTokenizer() {
    ...
    function tryNext(index) {
        ...
        if (index > 0) {
            statusEl.textContent = '形態素解析エンジンを読み込み中... (CDN経由)';
        }

        // タイムアウト: 15秒経過したら次のパスを試みる
        const timer = setTimeout(() => tryNext(index + 1), 15000);
...
function makeGroupKeyFromToken(token) {
    ...
    // 通常の場合：カタカナ→ひらがな変換＋全角→半角正規化した読みをキーに使用
    const normalizedReading = normalizeFullWidth(katakanaToHiragana(reading));
 ...
 * 表記揺れ候補のグループに属するトークンとブラケット一致箇所をハイライト表示する
 ...
 * @param {Array} bracketPositions - ブラケット一致位置一覧（detectBracketVariations の戻り値）
 */
function renderHighlightedText(text, tokens, highlightKeys, bracketPositions) {
    const container = document.getElementById('highlighted-text');
    if (!container) return;

    if (!text) { container.textContent = ''; return; }

    const hasBrackets = bracketPositions && bracketPositions.length > 0;
    if ((!tokens || tokens.length === 0) && !hasBrackets) {
    ...
    // --- Step 2: ブラケット位置と統合してソート ---
    const bpRanges = (bracketPositions || []).map(bp => ({ ...bp, source: 'bracket' }));
    const allRanges = [...tokenRanges, ...bpRanges].sort((a, b) => a.start - b.start);

    // --- Step 3: ブラケット出現箇所を事前登録 ---
    const occurrencesByKey = {};
    for (const bp of (bracketPositions || [])) {
        if (!occurrencesByKey[bp.key]) occurrencesByKey[bp.key] = [];
        occurrencesByKey[bp.key].push({
            id: bp.id, start: bp.start, end: bp.end, lineNumber: bp.lineNumber,
            snippet: bp.snippet, prefixIsStart: bp.prefixIsStart, suffixIsEnd: bp.suffixIsEnd
        });
    }
